...
Implementation adopted from: https://github.com/EBjerrum/SMILES-enumeration
"""
...
class EnumerationAugmenter(BaseAugmenter):
    """Enumeration Augmenter, vectorizer and devectorizer
    ...
    def augment(self, smiles: Union[str, List[str]], n_augment: int = 1) -> List[str]:
        """
        Performs enumeration augmentation on the canonical molecular SMILES

        Args:
            input_smiles (Union[str, List[str]]): List or string containing molecular SMILES.
            n_augment (int): Number of enumerations for each CHEMICAL SMILE. Defaults to 1.
        """
        assert len(smiles) > 0, "SMILES can not be empty!"
        try:
            if isinstance(smiles, str):
                smiles = [smiles]
            smiles = np.repeat(smiles, n_augment)
            self.fit(smiles, extra_chars=["\\"])
            v = self.transform(smiles)
            enumerated_smiles = self.reverse_transform(v)
        except:
            pass
